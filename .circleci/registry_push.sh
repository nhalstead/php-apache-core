...
# Build Image
docker build --pull --no-cache -t $IMAGE_PATH:$TAG_PREFIX$CIRCLE_BUILD_NUM $DOCKERFILE
...
docker tag $IMAGE_PATH:$TAG_PREFIX$CIRCLE_BUILD_NUM $IMAGE_PATH:${TAG_PREFIX}${CIRCLE_BUILD_NUM}
...
docker push $IMAGE_PATH:${TAG_PREFIX}${CIRCLE_BUILD_NUM}
